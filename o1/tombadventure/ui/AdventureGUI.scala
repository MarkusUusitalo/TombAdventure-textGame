  ...
  def top = new MainFrame:
    ...
    def updateInfo(info: String) =
      if !this.game.isOver then
        ...
        //if this.player.location.name == "Ambush Passage" then
          //this.turnOutput.text = info +
        this.turnOutput.text = this.game.goodbyeMessage
      if this.player.isHeadingIn then
        this.locationInfo.text = this.player.location.fullDescription(true)
      else
        this.locationInfo.text = this.player.location.fullDescription(false)
